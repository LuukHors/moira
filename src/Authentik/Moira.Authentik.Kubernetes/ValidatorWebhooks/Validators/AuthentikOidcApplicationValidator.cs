...
internal class AuthentikOidcApplicationValidator : AbstractValidator<AuthentikOidcApplication>
{
    ...
    private static bool HaveUniqueSecrets(IEnumerable<Secret>? secrets)
    {
        if (secrets is null)
        {
            return true;
        ...
        var keys = secrets.Select(secret =>
        {
            var cluster = secret.ClusterRef is null
                ? "local"
                : $"{secret.ClusterRef.Namespace}/{secret.ClusterRef.Name}/{secret.ClusterRef.Key}";
