...
internal class OIDCApplicationValidator : AbstractValidator<OidcApplication>
{
    ...
    public OIDCApplicationValidator()
    {
        ...
        RuleFor(a => a.Spec.ProviderSettingsRef)
            .ChildRules(settingsRef =>
            {
                settingsRef.RuleFor(t => t!.ApiVersion)
                    .NotEmpty()
                    .WithMessage("The \"providerSettingsRef.apiVersion\" property should be set");
                settingsRef.RuleFor(t => t!.Kind)
                    .NotEmpty()
                    .WithMessage("The \"providerSettingsRef.kind\" property should be set");
                settingsRef.RuleFor(t => t!.Name)
                    .NotEmpty()
                    .Matches(@"^[a-z0-9]([-a-z0-9]*[a-z0-9])?$")
                    .WithMessage("The \"providerSettingsRef.name\" property should be a valid Kubernetes name");
                settingsRef.When(t => !string.IsNullOrWhiteSpace(t!.Namespace), () =>
                {
                    settingsRef.RuleFor(t => t!.Namespace)
                        .Matches(@"^[a-z0-9]([-a-z0-9]*[a-z0-9])?$")
                        .WithMessage("The \"providerSettingsRef.namespace\" property should be a valid Kubernetes namespace");
                });
            })
            .When(a => a.Spec.ProviderSettingsRef is not null);
